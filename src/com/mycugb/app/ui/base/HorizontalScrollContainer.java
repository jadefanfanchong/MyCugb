package com.mycugb.app.ui.base;
...
* 负责界面左右滑动的容器类
 */
public class HorizontalScrollContainer extends ViewGroup
{
	...
	public HorizontalScrollContainer(Context context)
	{
		...
		mWidth = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 54, getResources()
				.getDisplayMetrics());

	}

	public HorizontalScrollContainer(Context context, AttributeSet attrs, int defStyle)
	{
		super(context, attrs, defStyle);
	}

	public HorizontalScrollContainer(Context context, AttributeSet attrs)
	{
		super(context, attrs);
	}
	...
	public boolean dispatchTouchEvent(MotionEvent ev)
	{
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (mTouchState == TOUCH_STATE_RESTART)
			{
				...
				if (x <= mWidth && mScreenState == SCREEN_STATE_CLOSE
						&& mTouchState == TOUCH_STATE_RESTART || x >= screenWidth - mWidth
						&& mScreenState == SCREEN_STATE_OPEN && mTouchState == TOUCH_STATE_RESTART)
				{
		...
		case MotionEvent.ACTION_UP:
			...
			if (mOnClick)
			{
				...
				mScroller.startScroll(getChildAt(1).getScrollX(), 0, -getChildAt(1).getScrollX(),
						0, 800);
				invalidate();
			}
			break;
			...
			if (mScrollState == SCROLL_STATE_ALLOW)
			{
				if (mVelocityValue > 2000)
				{
					...
					mScroller.startScroll(getChildAt(1).getScrollX(), 0,
							-(getWidth() - Math.abs(getChildAt(1).getScrollX()) -
